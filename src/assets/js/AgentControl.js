...
const tools = [
  {
    ...
    function: {
      ...
      description:
        "任何和当前时间有关的语句你都应该先调用此工具来获取当前的时间，根据反馈再自动调用后面工具。",
      parameters: {},
    },
  },
  {
...
const agentActionControl = {
  ...
  async finalAgent(prompt) {
    ...
    agentMsgControl.pushMsg(
      agentUtils.Visualization(msgObj, "str"),
      "assistant"
    );
    ...
    if (!("tool_calls" in assistantOutput)) {
      ...
      while (
        assistantOutput["tool_calls"] !== null &&
          ...
          if (assistantOutput.tool_calls[0].function.arguments) {
            try {
              ...
              const args = JSON.parse(
                assistantOutput.tool_calls[0].function.arguments
              );
        ...
        assistantOutput = (await this.getAgentResponse(messages)).choices[0]
          .message;
        if (Object.is(assistantOutput.content, null)) {
          assistantOutput.content = "";
        }
        // console.debug(`second assistantOutput:${JSON.stringify(assistantOutput.content)} ${JSON.stringify(assistantOutput.role)}`)
        agentQwenMsgControl.pushMsg(
          assistantOutput.content,
          assistantOutput.role
        );
        // agentMsgControl.pushMsg(agentUtils.Visualization(assistantOutput.content, 'str'), assistantOutput.role)
        const aMsg = {
          content: `${assistantOutput?.content}`,
          role: assistantOutput.role,
          stamp: getCurrentDateTime(),
        };
        agentMsgControl.pushMsg(
          agentUtils.Visualization(aMsg, "str"),
          "assistant"
        );
        messages.push(assistantOutput);
        i += 1;
        console.log(
          `第${i}轮大模型输出信息：${JSON.stringify(assistantOutput)}`
        );
      }
      agentRes = JSON.stringify(assistantOutput.content).replace(
        /^['"]|['"]$/g,
        ""
      );
    }
